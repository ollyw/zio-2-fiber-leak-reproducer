...
object Main extends App {
  override def run(args: List[String]) = fiberMetricsWorker *> poller.provideCustomLayer(httpClient).orDie.exitCode
...
//      _ = println(result)
  ...
  private def fiberMetricsWorker = ZIO.unit
//    (for {
//      roots <- Fiber.roots
//      total = roots.size
...
//    } yield ()).repeat(Schedule.fixed(5.seconds)).fork
  ...
  private val httpClient =
    ZManaged.runtime[ZEnv].flatMap { implicit rts =>
